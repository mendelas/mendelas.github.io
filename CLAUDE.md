...
`content/ja/` と `data/authors/me-ja.yaml` の文章は **すべて敬語（です・ます調）** で書く。
本文だけでなく、front matter の `summary`、bio、学歴・職歴・研究費・奨学金の
説明文も対象。

- である調（「〜している．」「〜である．」）で終えない。
- 体言止め（「〜の研究。」「〜を担当．」「〜にて実施。」）で終えない。
  「〜の研究です。」「〜を担当しました．」のように言い切る。
- 句読点は各ファイルの既存の表記（`，．` か `、。`）に合わせる。
- 論文タイトルや固有名詞は対象外（そのまま）。
