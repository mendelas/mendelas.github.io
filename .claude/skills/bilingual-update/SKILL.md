...
- **見出しは日本語版でも英語のまま。** メニュー名・ページタイトル・ブロック／セクション
  の `title`・`headings.*`・見出し用の i18n キー（`education` / `interests` /
  `experience` 等）は英語版と同じ英語にする。日本語にするのは本文と、ニュース記事・
  業績・経歴などの各項目の中身だけ。
- **日本語版の文章は敬語（です・ます調）で書く。** 本文・`summary`・bio・経歴や
  受賞の説明文すべてが対象。である調や体言止め（「〜の研究。」「〜を担当．」）で
  終えない。句読点は各ファイルの既存の表記（`，．` か `、。`）に合わせる。
